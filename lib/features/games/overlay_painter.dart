...
class OverlayPainter extends CustomPainter {
  ...
  void _drawKesatria(Canvas canvas, Size size) {
    for (var playerId in engine.drawnLandmarks.keys) {
      ...
      if (ls != null && rs != null) {
        double tx(double x) => _translateX(x, size);
        double ty(double y) => _translateY(y, size);
  ...
  void _drawFloatingTexts(Canvas canvas) {
    for (var ft in engine.floatingTexts) {
      final style = TextStyle(color: Colors.yellow.withValues(alpha: ft.alpha / 255), fontSize: 24, fontWeight: FontWeight.bold, shadows: const [Shadow(color: Colors.black, blurRadius: 4)]);
  ...
  void _drawBody(Canvas canvas, int id, Map<PoseLandmarkType, Offset> marks, double Function(double) tx, double Function(double) ty, double width) {
    ...
    final rsX = tx(rs.dx);
    final rsY = ty(rs.dy);
    final midShoulderX = (lsX + rsX) / 2;
    final midShoulderY = (lsY + rsY) / 2;

    double bodyHeight = width * 1.1;
    double midHipY = midShoulderY + width * 1.1; // fallback
    if (lh != null && rh != null) {
      midHipY = (ty(lh.dy) + ty(rh.dy)) / 2;
      bodyHeight = (midHipY - midShoulderY).abs() * 1.2;
    }

    // Anchor titik tengah antara bahu dan pinggul (pusat torso)
    final midX = midShoulderX;
    final midY = (midShoulderY + midHipY) / 2;

    final scaledW = width * 1.25;
    final scaledH = bodyHeight * 1.1;

    final angle = atan2(rsY - lsY, rsX - lsX);

    canvas.save();
    canvas.translate(midX, midY);
    canvas.rotate(angle);
    paintImage(canvas: canvas, rect: Rect.fromCenter(center: Offset.zero, width: scaledW, height: scaledH), image: img, fit: BoxFit.fill);
    canvas.restore();
  }
  ...
  void _drawShoulders(Canvas canvas, int id, Map<PoseLandmarkType, Offset> marks, double Function(double) tx, double Function(double) ty, double width) {
    ...
    // Di kamera depan (mirror), left/right tertukar secara visual
    // shoulder1_left = shoulder kiri pemain = tampil di KANAN layar (karena mirror)
    final imgScreenLeft  = AssetManager().getImage(id == 1 ? 'shoulder1_left'  : 'shoulder2_left');
    final imgScreenRight = AssetManager().getImage(id == 1 ? 'shoulder1_right' : 'shoulder2_right');
    if (imgScreenLeft == null || imgScreenRight == null) return;

    final padSize = width * 0.75;
    ...
    // Bahu kanan layar (= leftShoulder pose karena mirror kamera depan) → rsX,rsY
    canvas.save();
    canvas.translate(rsX, rsY);
    canvas.rotate(angle);
    paintImage(canvas: canvas, rect: Rect.fromCenter(center: Offset.zero, width: padSize, height: padSize), image: imgScreenLeft, fit: BoxFit.fill);
    canvas.restore();

    // Bahu kiri layar (= rightShoulder pose karena mirror) → lsX,lsY
    canvas.save();
    canvas.translate(lsX, lsY);
    canvas.rotate(angle);
    paintImage(canvas: canvas, rect: Rect.fromCenter(center: Offset.zero, width: padSize, height: padSize), image: imgScreenRight, fit: BoxFit.fill);
    canvas.restore();
  }
  ...
  void _drawHead(Canvas canvas, int id, Map<PoseLandmarkType, Offset> marks, double Function(double) tx, double Function(double) ty, double size) {
    ...
    double angle = 0.0;
    if (le != null && re != null) {
      // Di kamera depan sudah di-mirror X, jadi sudut antar mata langsung pakai re→le
      angle = atan2(ty(re.dy) - ty(le.dy), tx(re.dx) - tx(le.dx));
    }

    final scaledSize = size * 1.4;
    // Helm/kepala dinaikkan dari hidung ke atas (offset negatif Y = ke atas)
    const double headOffsetY = -0.45;
    ...
    paintImage(
      ...
      rect: Rect.fromCenter(
        center: Offset(0, scaledSize * headOffsetY),
        width: scaledSize,
        height: scaledSize,
      ),
      image: img,
